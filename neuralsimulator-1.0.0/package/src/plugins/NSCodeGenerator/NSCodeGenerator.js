...
//------------------------------------------------------------------------------------
    ...
    NSCodeGenerator.prototype.readSubjectNeuron=function(nodes){
        ...
        for (i=0;i<self.vM.length;i+=1){
            ...
            self.subjectNeuron=self.subjectNeuron.concat(neuronIndex+1);
        }
        function pathToIndex(path){
            var k=0;
            for (k=0;k<self.allComponent.length;k+=1){
...
//------------------------------------------------------------------------------------
     NSCodeGenerator.prototype.outputData=function(){
        var self=this;
        var fs = require('fs');
        var stream = fs.createWriteStream("MatlabSimulation.txt");
	    ...
	    for (i=0;i<self.allComponent.length;i+=1)
            ...
            //--------------------output simulation code------------------
                    ...
                    stream.write('SNN.v(i)=stimulus(i,2);'+'\n');
                    stream.write('else'+'\n');
                    stream.write('SNN.v(i)=0;'+'\n');
                    stream.write('end'+'\n');
                    stream.write('end'+'\n');
                    ...
                    stream.write('I_now(i+nr)=0.03*(W(3*i-2,:)+W(3*i-1,:)+W(3*i,:))*neuronVout(1:end,1);'+'\n');
                    stream.write('end'+'\n');
                    stream.write('I=I_now+I_last;'+'\n');
                    stream.write('I_last=I*0.98;'+'\n');
